...
use x86::apic::xapic::{XAPIC_SVR, XAPIC_TIMER_CURRENT_COUNT};
use x86::cpuid::CpuId;
...
impl LocalInterruptManager {
    pub fn new(physical_memory_offset: u64) -> Self {
            ...
            .unwrap_or_default()
        {
            ...
            let apic_base = unsafe { rdmsr(APIC_BASE) & 0xFFFFF000 };
    ...
    pub fn enable_timer(&self, vector_entry: u8, duration: Duration) {
        ...
        match self.r#type {
            LocalInterruptManagerType::XAPIC { base } => unsafe {
                ...
                loop {
                    ...
                    if u128::from((hpet.counter() - start) * counter_period_in_femtoseconds)
                        >= duration.as_millis() * 10u128.pow(12)
                    {
                        break;
                    }
                }
